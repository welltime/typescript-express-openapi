    ...
    logException(topic, severity, exception, subtopic = SubTopic.Unknown) {
        try {
            if (!constants_1.is_dev_env) {
                ...
                this.log(chalk_1.default.red.bold(`topic: ${topic}\nsubTopic: ${subtopic}\nErrorMessage: ${exception.message}\nseverity: ${severity}\nstack: ${exception.stack}\n`));
            }
        }
        catch (e) {
            this.log({
    ...
    logMessage(topic, severity, message, subtopic = SubTopic.Unknown) {
        try {
            if (!constants_1.is_dev_env) {
                ...
                this.log(chalk_1.default.green.bold(`logType: ${LogType.message} \nmessage: ${message} \n`));
            }
        }
        catch (e) {
            this.log({
    ...
    logObject(topic, severity, logObj, subtopic = SubTopic.Unknown) {
        try {
            if (!constants_1.is_dev_env) {
                ...
                this.log(chalk_1.default.green.bold(`logType: ${LogType.object} \nmessage: ${JSON.stringify(logObj)} \n`));
            }
        }
        catch (e) {
            this.log({
